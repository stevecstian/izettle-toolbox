...
public class QueueServiceSenderTest {
    ...
    public void postMessageShouldWork() throws Exception {
        when(mockAmazonSQS.sendMessageAsync(any(SendMessageRequest.class))).thenReturn(CompletableFuture
            .completedFuture(mock(SendMessageResult.class)));
        messageQueueProducer.post(new TestMessage("Hello!"));
        verify(mockAmazonSQS, times(1)).sendMessageAsync(any(SendMessageRequest.class));
    }
    ...
    public void shouldSendBatchesInSizeOfTen() throws Exception {
        ...
        verify(mockAmazonSQS, times(1)).sendMessageBatchAsync(captor.capture());
        assertThat(captor.getValue().getEntries()).hasSize(10);

        messagePublisher.postBatch(messageBatch(20), subject);
        verify(mockAmazonSQS, times(3)).sendMessageBatchAsync(any(SendMessageBatchRequest.class));

        messagePublisher.postBatch(messageBatch(11), subject);
        verify(mockAmazonSQS, times(5)).sendMessageBatchAsync(captor.capture());
        assertThat(captor.getValue().getEntries()).hasSize(1);

        messagePublisher.postBatch(messageBatch(9), subject);
        verify(mockAmazonSQS, times(6)).sendMessageBatchAsync(captor.capture());
    ...
    public void postBatchShouldSendMessagesWithSNSEnvelope() throws Exception {
        ...
        // Assert
        verify(mockAmazonSQS).sendMessageBatchAsync(captor.capture());
    ...
    public void postAsSNSMessageShouldSendMessagesWithSNSEnvelope() throws Exception {
        ...
        // Assert
        verify(mockAmazonSQS).sendMessageBatchAsync(any(SendMessageBatchRequest.class));
    }
